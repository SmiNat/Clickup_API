...
class CreateTaskFullRequest(TaskBasicRequest):
    assignees: list[int | None] | None = Field(
        default=None, title="Assignees ids", examples=[[None]]
    )
...
class UpdateChecklist(BaseModel):
    ...
    position: int | None = Field(
        default=None,
        description="Position refers to the "
...
class UpdateChecklistItem(BaseModel):
    name: str | None = Field(default=None, min_length=1)
    assignee: int | None = Field(default=None, examples=[None])
    remove_assignee: bool = Field(
        default=False,
        description="To remove Assignee from "
    ...
    parent: str | None = Field(
        default=None,
        description="To nest a checklist item "
...
async def create_checklist(
    ...
    response = requests.post(
        url, headers=header(token), params=query, json=jsonable_encoder(name)
    )
    if not response.status_code < 400:
        raise HTTPException(response.status_code, response.json())
    return response.json()
...
async def edit_checklist(
    checklist_id: str, name: UpdateChecklist, token: str | None = None
):
    validate_token(token)
...
@router.post(
    "/checklist/{checklist_id}/checklist_item", status_code=status.HTTP_201_CREATED
)
async def create_checklist_item(
    checklist_id: str, item: CreateChecklistItem, token: str | None = None
):
    validate_token(token)
